    ...
    <div className="grid min-w-0 grid-cols-2 gap-x-3 gap-y-2 md:flex md:flex-wrap md:items-end">
      ...
      <label className="col-span-2 min-w-0 md:col-span-1">
        ...
        <input
          ...
          className="input min-w-0 max-w-full w-full md:w-40"
        />
      </label>
      <label className="min-w-0">
      ...
      <label className="min-w-0">
        <span className="field-label">Pushed within</span>
        <span className="relative block">
          <select
            value={pushedSinceDays}
            onChange={(e) => onChange({ pushedSinceDays: Number(e.target.value) })}
            className="select min-w-0 max-w-full w-full md:w-36"
          >
            <option value={0}>any time</option>
            <option value={7}>7 days</option>
