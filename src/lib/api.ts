...
export async function fetchSearch(
	params: SearchParams,
): Promise<{ items: Repo[]; total: number }> {
	return getJson(`/search${toQuery(params)}`);
}

export async function fetchRisers(
	window: "1d" | "7d" | "30d",
	page = 1,
): Promise<{ items: Repo[]; total: number }> {
...
export async function fetchPulse(
	...
	return getJson(`/pulse${toQuery({ since, limit })}`);
}
...
export async function fetchReadme(
	...
	return getJson(
		`/repo/${encodeURIComponent(owner)}/${encodeURIComponent(name)}/readme`,
	);
}

export async function fetchFavourites(
	window: "1d" | "7d" | "30d",
): Promise<{ items: WatchlistRepo[]; total: number }> {
...
export async function updateFavourites(
	...
	return sendJson("PATCH", "/favourites", { ids, patch });
}
